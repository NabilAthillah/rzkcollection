...
@section('content')
    ...
    <form action="{{ route('register.user') }}" class="flex flex-col justify-center items-center mt-10" method="POST">
        @csrf
        <p class="cursor-default font-['NotoSerifDisplay'] font-black text-4xl mb-8 uppercase">register</p>
        <div class="container relative h-96 w-2/5 border-[1px] border-black flex flex-col px-10 py-8">
            ...
            @foreach ($errors->all() as $err)
                <div class="alert alert-danger text-end">
                    <p class="font-black text-center items-center" style="color: red">{{$err}}</p>
                </div>
            @endforeach
            @endif
            ...
            <div class="form-group flex flex-col font-['Lora']">
                ...
                <input type="email" name="email" id="email" class="h-7 mb-2 form-control border-[1px] border-[#848992] pl-2" required>
            </div>
            ...
            <a href="/user/login" class="absolute bottom-4 right-4 font-['Lora'] text-[#4c4e53] italic">Already Have Account? Login</a>
        </div>
    </form>
    <script>
